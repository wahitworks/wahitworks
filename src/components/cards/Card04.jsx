...
function BookmarkItem({ region, stationName, nickname }) {
  ...
  // *기존 상수 폴더에서 제거한 변수 여기 컴포넌트에서 재 정의
  const gradeClass = { 1: "good", 2: "normal", 3: "bad", 4: "very-bad" };
  ...
  const getGradeNumberFromValue = (value, type) => {
    const standards = {
      ...
      O3: { good: 0.03, moderate: 0.09, bad: 0.15 },
    };
  ...
  const data = useSelector((state) =>
    stationName ? state.fineDust.data[stationName] : null
  );
  const loading = useSelector((state) =>
    stationName ? state.fineDust.loading[stationName] : false
  );
  const error = useSelector((state) =>
    stationName ? state.fineDust.error[stationName] : null
  );
  ...
  const pm10ClassName = gradeClass[dustGrade];
  const pm25ClassName = gradeClass[ultraDustGrade];
  const o3ClassName = gradeClass[o3Grade];

  // 등급별 로고
  const AirQualityLogo = ({ grade }) => {
    switch (grade) {
      case "1":
        return <LogoGood animated />;
      case "2":
        return <LogoModerate animated />;
      case "3":
        return <LogoBad animated />;
      case "4":
        return <LogoVeryBad animated />;
      default:
        return <LogoError animated />;
    }
  };
  ...
  return (
    <div className="card04-bookmark-item-container">
      {/* 클릭시 토글 */}
      <div
        className={`card04-bookmark-list ${listToggle ? "toggled" : ""}`}
      ...
      >
        ...
        {nickname ? (
          ...
          <div className="card04-bookmark-list-title">
            <p>{nickname}</p>
        ...
        <div className="card04-bookmark-list-triangle">
      ...
      {listToggle && (
        <motion.div
          className={`card04-bookmark-list-toggle ${listToggle ? "show" : ""}`}
        ...
        >
          ...
          >
            {card04Error ? (
              // 에러 : 에러 메시지
              <div className="card04-bookmark-list-comment-err">
                정보를 가져올 수 없습니다.
              </div>
            ) : card04Loading ? (
              // 로딩 중 : 로딩 메시지
              ...
              <>
                <div className="card04-bookmark-list-toggle-top">
                  <p className="card04-bookmark-list-toggle-station">
                    {stationName} 측정소
                  </p>
                </div>
                <div className="card04-bookmark-list-toggle-container">
                  <div className="card04-bookmark-list-toggle-info">
                    <p className="card04-bookmark-list-toggle-info-title">
                      미세먼지
                    </p>
                    <p className="card04-bookmark-list-toggle-info-value">
                      {dustValue}㎍/㎥
                    </p>
                    <div className="card04-air-log">
                      <AirQualityLogo grade={dustGrade} />
                    </div>
                    <p className="card04-list-toggle-ingo-text">
                      {getGradeText(dustGrade)}
                    </p>
                  </div>
                  <div className="card04-bookmark-list-toggle-info">
                    <p className="card04-bookmark-list-toggle-info-title">
    ...
    <div className="card04-container">
      ...
      {bookmarkedRegions && bookmarkedRegions.length > 0 ? (
        ...
        <div className="card04-bookmark-list-none">
          <p className="card04-bookmark-list-comment">
            즐겨찾기된 장소가 없습니다.
          </p>
        </div>
      )}
